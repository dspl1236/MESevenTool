...
class TestPatches:
    def test_all_patches_valid(self):
        from meseventool.patches import OffsetPatchDef, MultiOffsetPatchDef, FixedAddressPatchDef
        for p in ALL_PATCHES:
            assert p.name
            assert p.description
            assert isinstance(p.category, PatchCategory)
            # Needle/mask only on needle-based patches
            if isinstance(p, (OffsetPatchDef, MultiOffsetPatchDef, FixedAddressPatchDef)):
                # Offset/fixed patches: just check stock/patch length
                if isinstance(p, (OffsetPatchDef, FixedAddressPatchDef)):
